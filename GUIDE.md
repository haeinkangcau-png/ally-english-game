...
> ⚠️ **정책 (2026-07 변경): 워크시트는 로컬 전용.**
> - 단어를 추가할 때 trace/look 워크시트를 **자동 생성하지 않는다.** 필요할 때만 요청 시 생성.
> - 모든 워크시트(`*_look.html`, `*_trace.html`, `special_trace*.html`)는 **git에 올리지 않는다** — `.git/info/exclude`로 제외. 로컬에서만 관리·인쇄.
> - 스페셜 워크시트는 기존 파일을 덮어쓰지 말고 **날짜 붙인 새 파일**로 생성 (예: `special_trace_20260712.html`).
> - 게임 데이터(`words.js`)·`index.html`만 커밋/업로드한다.
